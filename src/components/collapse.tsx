...
const Collapse: FC<CollapseProps> = ({
  ...
  return (
    <div className="border rounded-md flex flex-col w-full">
      <div className="h-12 flex gap-x-3 px-4 items-center justify-between cursor-pointer">
        <input
          className="flex-1"
          value={header}
          onChange={(e) => handleKeyChange(path, e.target.value, value)}
        />{" "}
        {filled}/{total}
        <div className="flex items-center gap-x-2">
          <button
            onClick={() => addKey(path)}
            className="h-8 flex items-center justify-center w-8 rounded-full hover:bg-gray-100"
          >
            +
          </button>
          <button
